...
## PROBLEMS:
## (1) If user removes a parameter from a model, the corresponding coefficient isn't removed from the list of coefficients to model.
## This is because while the list of UI widgets that is generated by lapply(...) that is getting passed to output changes, so UI looks as if
## parameters are being removed, the overall input object is not aware that the user no longer wants the removed parameters.
## https://groups.google.com/forum/#!topic/shiny-discuss/u36gmSZzrvA
## (2) Coordinating subscripting with model coefficients.

## SOLUTION:
## (1) Try using reactiveValues instead of input to store parameter list: http://stackoverflow.com/questions/31454185/how-to-add-remove-input-fields-dynamically-by-a-button-in-shiny
## (2) In GatherModelsHLM, for level j, use level j-1's coefficient labeling to help organize how model storage list is comprised, and to help write the coefficient's target variable.
...
shinyServer(function(input, output, session) {
	...
	##----------------- PRINT COEFFICIENT MODELS BY LEVEL, BUILD MODEL MATRIX  ---------------##
